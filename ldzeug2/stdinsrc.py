...
def ldproject_pipe(
    ld_decode_project: str, file_handle=sys.stdin.buffer
) -> tuple[vs.VideoNode, LDDProject]:
    project = LDDProject(ld_decode_project)

    # orig_tbc,project = ldproject_frames(ld_decode_project,addjson,extension)

    tbc = streaming_in(file_handle).std.AssumeFPS(fpsnum=60_000, fpsden=1001)[
        : len(project.fields_g16)
    ]
    tbc = tbc.resize.Bicubic(format=vs.GRAYS, range_in=1, range=1)
    tbc = core.std.DoubleWeave(tbc, tff=True)[::2]
    tbc = core.std.CopyFrameProps(
        tbc, project.frames, props=["PhaseID_A", "PhaseID_B", "fieldPhaseID"]
    )
    return tbc, project
...
def streaming_in(file_handle, ww=910, hh=263):
    ...
    blk_clp = core.std.BlankClip(
        format=vs.GRAY16, width=ww, height=hh, length=200000000
    )
    clp = blk_clp
