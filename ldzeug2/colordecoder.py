...
from vstools import color, core, set_output, vs,split,join
...
def color_notch_bp(framesp: vs.VideoNode,f1: float = 4.5,f2: float = 5.5) -> vs.VideoNode:
...
# for 4fsc
c_colorlp_b = [
	2.236562025869846e-03, 9.679572273064329e-03, 6.100849475810623e-03, -2.082153208645807e-02, 
	-4.872917723725065e-02, -1.535300561979003e-02, 1.137084573789944e-01, 2.775133160099456e-01, 
	3.533666167131518e-01, 2.775133160099456e-01, 1.137084573789944e-01, -1.535300561979003e-02, 
	-4.872917723725065e-02, -2.082153208645807e-02, 6.100849475810623e-03, 9.679572273064329e-03, 
...
def adjust_y(signal: Stackable,
             i: Stackable,
             q: Stackable,
             phase_fliped: Stackable) -> Stackable:
    return signal - (iq_phase.switch([
        (0, -q),
        (1, i),
        (2, q),
        (3, -i)
...
def to_rgb(y: Stackable,
           u: Stackable,
           v: Stackable,
           consts:CombConsts) -> tuple[Stackable,Stackable,Stackable]:
...
def comb_split_already(vid_v: vs.VideoNode,
                      chrma: vs.VideoNode,
    ...
    if color_low:
        if isinstance(color_low,bool):
            color_low =4.2
        import scipy.signal as sp
        fltrr = sp.firwin(127,color_low,fs=(315/88)*4,window="boxcar",pass_zero="lowpass")
...
def split_2d(sm,vid_x: Stackable,*,consts:CombConsts,chr1d: Optional[Stackable] = None) -> Stackable:
    if chr1d is None:
...
def split_3d(sm, vid_x: Stackable,*, consts:CombConsts,chr2d_override: vs.VideoNode = None):
    @dataclass
    class Reference:
        base: Stackable
        luma: Stackable
    ...
    def compare_candidate(ref: Reference, can: Reference):
        ...
        weights = [0.5, 1.0, 0.5]
        for offset in range(-1,2):
            iqPenalty += (ref.chroma - (-can.chroma))[offset,0].abs() * weights[offset + 1]
    ...
    d2_nxt_1up   = compare_candidate(origin_ref,mkref(vx_nxt, d2_nxt, (0, 1))) + FIELD_BONUS
    d2_prv_1down = compare_candidate(origin_ref,mkref(vx_prv, d2_prv, (0,-1))) + FIELD_BONUS
    ...
    smlst = Stackable.choose_smallest(sm,[
        (d1_left,  d2),
        (d1_right, d2),
        (d2_2up,   d2),
        (d2_2down, d2),
...
def combyc_field_model(vid_v: vs.VideoNode, network_path = None,backend = None,**kwargs):
    ...
    lma_field = inference(mdl_in, network_path=network_path, backend=backend).std.DoubleWeave(tff=True)[::2]


    return comb_split_already(
        vid_v,
        core.akarin.Expr([vid_v,lma_field],"x y -"),
        **kwargs
    )
...
def comb3d(vid_v: vs.VideoNode,*,consts=CombConsts(True),chr2d_override=None,**kwargs):
    sm= StackableManager()
    vid_x = sm.add_clip(vid_v)
    chrm3d = sm.eval_x(split_3d(sm,vid_x,consts=consts,chr2d_override=chr2d_override))
...
def comb_color_cnn(frames,
        ...
        crop=False,
        project: Optional[LDDProject]=None,
        bp_q=False,
        backend=None,
        chroma_phase=0.0,
