...
class Stackable:
        ...
        if len(item) == 2:
            ...
            if self.mode == 0:
                ...
                return Stackable(
                    ...
                    (
                        self.contents[0],
                        self.contents[1] + item[0],
                        self.contents[2] + item[1],
                    ),
                )
            elif self.mode == 2:
                return self
    ...
    def iftrue(self, then, elsee):
        return Stackable(
            ...
            [
                self,
                Stackable.const_or_stack(then),
                Stackable.const_or_stack(elsee),
                Stackable(2, "?"),
            ],
        )
    ...
    def clip(self, mina, maxa):
        return Stackable(
            ...
            [
                self,
                Stackable.const_or_stack(mina),
                Stackable.const_or_stack(maxa),
                Stackable(2, "clip"),
            ],
        )
    ...
    def max(self, other):
        return Stackable(
            0, [self, Stackable.const_or_stack(other), Stackable(2, "max")]
        )

    def min(self, other):
        return Stackable(
            0, [self, Stackable.const_or_stack(other), Stackable(2, "min")]
        )
    ...
    def __and__(self, other):
        return Stackable(
            0, [self, Stackable.const_or_stack(other), Stackable(2, "and")]
        )
...
class StackableManager:
    ...
    @overload
    def eval_x(self, s: Stackable) -> Stackable: ...

    @overload
    def eval_x(self, s: list[Stackable]) -> tuple[Stackable, ...]: ...
